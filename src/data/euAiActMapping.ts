...
export const MAPPING_SOURCE_NOTE =
  "Status is scored against the public repos, not README claims. Porto Seguro publishes a committed conformity JSON plus screenshots — pipeline files named in that README are not in the tree. Enterprise AI Risk implements a hash-chained ledger, unique decision IDs, and Jira human gates. Rekor and S3 Object Lock exist as optional sinks and are off by default. Copy is original.";
...
export const MAPPING_STATUS_LEGEND =
  "Status refers to the reference implementation—not the legal applicability of the obligation.";

export const MAPPING_LEAD_SHORT =
  "Role and risk classification determine which obligations apply. This is an engineering-focused mapping, not legal advice.";

export const MAPPING_PHASED_DATES_NOTE =
  "Application is phased by category and role. Art. 50 transparency obligations are widely described as applying from August 2026; Annex III high-risk duties are often described from December 2027. This page is the control and evidence design expected when those obligations apply — not a determination of your dates.";
...
export const EU_AI_ACT_MAPPING_ROWS: MappingRow[] = [
  {
    ...
    control: "Designed KS drift gates with ALLOW/BLOCK thresholds",
    evidence: "Drift snapshot in the committed conformity JSON — no KS/gate code in the public repo",
    status: "Reference",
    requirement:
  ...
  },
  {
    ...
    evidence: "Training-data card and fairness snapshot in the conformity JSON — no lineage or split-validation code",
    status: "Reference",
    requirement:
...
record(fairness.demographic_parity, fairness.equalized_odds)`,
  ...
  },
  {
    ...
    roles: ["provider", "high-risk"],
    systemType: "Provider · high-risk",
    controlKind: "compliance-as-code",
    control: "Committed model-card fields in a versioned conformity JSON",
    evidence: "eu_ai_act_conformity_v1.json — not generated by a pipeline in the public repo",
    status: "Reference",
    requirement:
...
assert pack.model_details.version == release.git_sha`,
  ...
  },
  {
    ...
    evidence: "previous_hash chain + canonical SHA-256 DecisionRecord. Rekor/S3 optional, off by default",
    status: "Implemented",
    requirement:
...
store.append(event)  # retain per category; ≥6 months for many high-risk systems (Arts 19/26)`,
  ...
  },
  {
    ...
    evidence: "JSON flags and README screenshots — no exportable global/local explanation artefacts",
    status: "Reference",
    requirement:
...
return score, explain  # never a silent decision`,
  ...
  },
  {
    ...
    evidence: "HMAC-verified Jira gate closures with allow-listed actor and timestamp",
    status: "Implemented",
    requirement:
  ...
  },
  {
    ...
    control: "Hold-out and perturbation metrics recorded in the pack",
    evidence: "Adversarial snapshot in the conformity JSON — no CI-blocking perturbation tests",
    status: "Reference",
    requirement:
...
fail_pipeline() if not robustness_passed`,
  ...
  },
  {
    ...
    control: "Independent stop-path / kill switch (not in the reference repos)",
    evidence: "No executable kill switch; Porto Seguro lists agentic runtime controls as out of scope",
    status: "Planned",
    requirement:
  ...
  },
  {
    ...
    systemType: "Provider",
    controlKind: "compliance-as-code",
    control: "Versioned YAML / compliance-as-code profiles in CI",
    evidence: "rules/*.yaml + CI evidence-pack JSON with SHA-256 deploy gate",
    status: "Implemented",
    requirement:
...
ci.upload("evidence-pack.json")`,
  ...
  },
  {
    ...
    evidence: "No deployer runbook in the reference repos — named oversight log is next work",
    status: "Planned",
    requirement:
...
monitor(drift_and_incidents)`,
  ...
  },
  {
    ...
    systemType: "Provider · high-risk",
    controlKind: "SDLC gate",
    control: "Annex IV–style fields in a committed documentation pack",
    evidence: "Conformity JSON only — no CI SHA-256 export of an Annex IV set in that repo",
    status: "Reference",
    requirement:
